...
export default function AdminExperiencesPage() {
  ...
  const [bulkTheme, setBulkTheme] = useState(0);
  ...
  async function handleCreateSticker() {
    if (!stickerForm.code.trim() || !stickerForm.targetImage.trim()) {
      setError("Code and target image URL are required");
      return;
    }
    setSaving(true);
    setError("");
  ...
  const filteredKeychains = keychains.filter(
    (k) => !search || k.code.includes(search)
  );
  const filteredStickers = stickers.filter(
    (s) => !search || s.code.includes(search) || s.message?.includes(search)
  );
    ...
    >
      ...
      {qrModal && (
        ...
        >
          ...
          >
            ...
            {qrModal.type === "keychain" &&
              (() => {
                const t = getTheme(resolveThemeId(qrModal.theme));
                return (
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    ...
                    />
                    <span
                      style={{ fontFamily: FONT, fontSize: 10, color: "#555" }}
                    >
                      {t.name} design
                    </span>
                  </div>
            ...
            >
              <button
                onClick={() => downloadQR(qrModal.code, qrModal.type)}
                style={{
                  padding: "10px 20px",
                  background: "#000",
                  border: "none",
      ...
      >
        <div>
          ...
          <h1 style={{ fontFamily: SERIF, fontSize: 32, fontWeight: 300 }}>
            Experiences
          </h1>
        </div>
      ...
      >
        {[
          ["keychains", `Keychains (${keychains.length})`],
          ["stickers", `Stickers (${stickers.length})`],
        ].map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            style={{
              ...
              border: "none",
              borderBottom:
                tab === key ? "2px solid #00E5FF" : "2px solid transparent",
      ...
      {showBulkForm && (
        ...
        >
          ...
          >
            ...
            >
              <div style={{ fontFamily: SERIF, fontSize: 24, fontWeight: 300 }}>
                Bulk Generate
              </div>
              <button
            ...
            >
              Select the physical keychain design you are printing QR codes for.
              This must match the target image index in master-themes.mind.
            </p>
            {error && (
              <div
                style={{
            ...
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <div>
                <label style={labelStyle}>PHYSICAL KEYCHAIN DESIGN</label>
                {/* Swatch picker — auto-populated from KEYCHAIN_THEMES */}
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  {KEYCHAIN_THEMES.map((t) => (
                    <button
                      key={t.id}
                      onClick={() => setBulkTheme(t.id)}
                      title={t.name}
                      style={{
                        width: 40,
                        height: 40,
                        ...
                        border:
                          bulkTheme === t.id
                            ? "3px solid #00E5FF"
                            : "3px solid rgba(242,240,235,0.15)",
                        cursor: "pointer",
                        boxShadow:
                          bulkTheme === t.id ? "0 0 0 2px #00E5FF44" : "none",
                ...
                >
                  Selected:{" "}
                  <span style={{ color: getTheme(bulkTheme).bg }}>
                    {getTheme(bulkTheme).name}
                  </span>{" "}
                  (index {bulkTheme})
                </div>
              </div>
              <button
                ...
                style={{
                  padding: "14px",
                  background: generating ? "rgba(255,255,255,0.06)" : "#00E5FF",
                  border: "none",
                  borderRadius: 10,
                  color: generating ? "rgba(242,240,235,0.3)" : "#000",
                  fontSize: 12,
                  fontWeight: 700,
                  letterSpacing: "0.1em",
                ...
                >
                  {generated.map((code) => (
                    <button
                      key={code}
                      onClick={() =>
                        setQrModal({ code, type: "keychain", theme: bulkTheme })
                      }
                      style={{
                        padding: "6px 12px",
      ...
      {showStickerForm && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: "grid", gap: 16 }}>
              {[
                ["UNIQUE CODE", "code", "e.g. stk-dragon-001"],
                [
                  "TARGET IMAGE URL (.mind file)",
                  "targetImage",
                  "https://…/targets.mind",
                ],
                [
                  "3D ASSET URL (.glb, optional)",
                  "assetUrl",
                  "https://…/model.glb",
                ],
              ].map(([lbl, key, ph]) => (
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setStickerForm((f) => ({ ...f, message: e.target.value }))
                  }
                  placeholder='Optional AR overlay text'
                />
              </div>
              <button
                ...
                style={{
                  padding: "14px",
                  background: saving ? "rgba(255,255,255,0.06)" : "#00E5FF",
                  border: "none",
                  borderRadius: 10,
                  color: saving ? "rgba(242,240,235,0.3)" : "#000",
                  fontSize: 12,
                  fontWeight: 700,
                  letterSpacing: "0.1em",
      ...
      {tab === "keychains" &&
        (loading ? (
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "rgba(242,240,235,0.4)" }}>
              Generate bulk QR codes to get started.
            </div>
          </div>
        ) : (
          <div
          ...
          >
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "120px 140px 80px 60px 1fr 120px 80px",
                gap: 12,
                padding: "10px 20px",
            ...
            {filteredKeychains.map((k, i) => {
              const t = getTheme(resolveThemeId(k.theme));
              return (
                <div
                  key={k.id}
                  style={{
                    ...
                    alignItems: "center",
                    borderBottom:
                      i < filteredKeychains.length - 1
                        ? "1px solid rgba(242,240,235,0.04)"
                        : "none",
                    opacity: k.isActive ? 1 : 0.45,
                  }}
                >
                  <div style={{ fontSize: 11, color: "#00E5FF" }}>{k.code}</div>
                  <div
                  ...
                  >
                    ...
                    />
                    <span
                      style={{ fontSize: 10, color: "rgba(242,240,235,0.6)" }}
                    >
                      {t.name}{" "}
                      <span style={{ color: "rgba(242,240,235,0.3)" }}>
                        #{k.theme}
                      </span>
                    </span>
                  </div>
                  <div>
                    {k.claimed ? (
                      <span style={{ fontSize: 9, color: "#64c864" }}>
                        ✓ YES
                      </span>
                    ) : (
                      <span
                        style={{ fontSize: 9, color: "rgba(242,240,235,0.3)" }}
                      >
                        — NO
                      </span>
                    )}
                  ...
                  <div
                    style={{ fontSize: 10, color: "rgba(242,240,235,0.35)" }}
                  >
                    {new Date(k.createdAt).toLocaleDateString("en-IN")}
                  </div>
                  <button
                    onClick={() => toggleKeychain(k.id, k.isActive)}
                    style={{
                      padding: "5px 12px",
                      borderRadius: 20,
                      fontSize: 9,
      ...
      {tab === "stickers" &&
        (loading ? (
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "rgba(242,240,235,0.4)" }}>
              Create a sticker experience to get started.
            </div>
          </div>
        ) : (
          <div
          ...
          >
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "140px 1fr 1fr 60px 120px 80px",
                gap: 12,
                padding: "10px 20px",
                borderBottom: "1px solid rgba(242,240,235,0.05)",
                fontSize: 9,
            ...
            {filteredStickers.map((s, i) => (
              ...
              >
                ...
                <button
                  onClick={() => toggleSticker(s.id, s.isActive)}
                  style={{
                    padding: "5px 12px",
                    borderRadius: 20,
                    fontSize: 9,
                ...
                <button
                  onClick={() => setQrModal({ code: s.code, type: "sticker" })}
                  style={{
                    padding: "5px 10px",
                    borderRadius: 6,
                    fontSize: 9,
